function [negloglikelihood] = Estimate_Stangl2(beta, valpha, g2, g3, sigma, nu, Input, config)
...
for tr = 1:sampleSize
    ...
    if config.includeStand==true
        men_length1 = l1*valpha*(1-exp(-beta*durationL1))/(beta*durationL1)*exp(-beta*(durationL2+durationStand));
    else
        men_length1 = l1*valpha*(1-exp(-beta*durationL1))/(beta*durationL1)*exp(-beta*durationL2);
    end
    ...
    men_length2 = l2*valpha*(1-exp(-beta*durationL2))/(beta*durationL2);
