function [negloglikelihood] = Estimate_alpha_k_g2_g3_m3_sigma_nu(alphap, k, g2, g3, m3, sigma, nu, Input, config)
% Zilong Ji, UCL, 2022, zilong.ji@ucl.ac.uk
% leaky integration with distance (added because one of the reviewers asked)
% Find the likelihood with parameters specified in the model function name
% Args (in common between all estimates functions):
% alphap: decay factor for the mental distance (calculation error)
...
for tr = 1:sampleSize
    ...
    if config.includeStand==true
        men_length1 = k*(1-exp(-alphap*l1))/alphap*exp(-alphap);
    else
        men_length1 = k*(1-exp(-alphap*l1))/alphap*exp(-alphap*l2);
    end
    ...
    men_length2 = k*(1-exp(-alphap*l2))/alphap;
