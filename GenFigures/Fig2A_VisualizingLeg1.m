...
function plotLeg1(AllParams, AllDX, config)
    ...
    for TRIAL_FILTER=1:numConds
        ...
        set(gca, ...
            ...
            'XLim'        , [0.5, 2.5],...
            'YLim'        , [min([all_Leg1,all_Leg1_prime])-0.1,max([all_Leg1,all_Leg1_prime])+0.1],...
            'LineWidth'   , .5        );
            %'Ytick'       , [1:ceil(max([all_Leg1,all_Leg1_prime]))],...
            %'YLim'        , [0,ceil(max([all_Leg1,all_Leg1_prime]))],...
        title('Outbound leg 1');
        %% save figure
        exportgraphics(f,config.ResultFolder+"/Leg1_"+conditionName{TRIAL_FILTER}+".png",'Resolution',300);
        exportgraphics(f,config.ResultFolder+"/Leg1_"+conditionName{TRIAL_FILTER}+".pdf",'Resolution',300, 'ContentType','vector');
    end
end
