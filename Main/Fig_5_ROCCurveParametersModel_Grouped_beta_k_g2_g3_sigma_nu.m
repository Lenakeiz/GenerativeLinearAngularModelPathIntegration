...
disp("AUC std (beta k g2): " + num2str(AUC.std));
drawnow;
...
disp("AUC std (g3): " + num2str(AUC.std));
drawnow;
...
function AUCOut = plotROCCurveSVM(param1, param2, param1Label, param2Label, paramColor)
    ...
    for i =1:M
        ...
        %cv = cvpartition(allLabels,'Holdout',holdout);
        mdl  = fitcsvm(allData,allLabels,"ClassNames",[{param1Label},{param2Label}], CrossVal="on", Holdout=holdout, KernelFunction="linear");
        ...
        [fittedLabels,post_probabilities] = predict(comp_mdl_post,Xtest);
