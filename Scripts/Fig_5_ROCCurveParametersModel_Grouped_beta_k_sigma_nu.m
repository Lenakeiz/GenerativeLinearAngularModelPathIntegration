...
config.ResultFolder = pwd + "/Output/ModelFigures/Fig5/ModelParameters/Grouped_beta_k_sigma_nu";
...
legendText{1,1} = "AUC(" + convertCharsToStrings({'\beta k \sigma \nu'}) + ") = " + num2str(round(AUC.mean,2),2);
disp("AUC std (all params): " + num2str(AUC.std));
drawnow;

AUC = plotROCCurveSVM(params1(:,[1 2]), params2(:,[1 2]), params1groupName, params2groupName, config.color_scheme_npg(8,:));

legendText{1,2} = "AUC(" + convertCharsToStrings({'\beta k'}) + ") = " + num2str(round(AUC.mean,2),2);
