...
%% Plotting individual linear regressions
% Getting the indices in an automatic way
...
statuscode = MRIModelParamsDataTable(:,find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"CSF")));
sex        = MRIModelParamsDataTable(:,find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"Sex")));
age        = MRIModelParamsDataTable(:,find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"Age")));

mri_indeces = [...
    find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"norm_ErC"))...
    find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"norm_PosMed"))...
    find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"norm_AntLat"))...
    find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"norm_hippocampus"))...
    find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"norm_superiorparietal_volume_Dest"))...
    find(strcmp(MRIModelParamsDataTable.Properties.VariableNames,"norm_inferiorparietal_volume_Dest"))...
];

for param = 1:length(config.ParamName)
    for mri = 1:length(config.ParamName)
        plotLinearRegression()
    end
end

%% Plotting a linear regression using a linear model
function plotLinearRegression(x,y,sex,age,statuscode)
